...
class NeuronAI {
    ...
    calculateMath(message) {
        try {
            ...
            if (!this.isValidMathExpression(expression)) {
                this.sendResponse('Не могу решить это выражение. Попробуйте что-то проще, например: "посчитай 2+2" или "сколько будет 15*3"');
                return;
            }
            ...
            if (result === null || isNaN(result)) {
                this.sendResponse('Не могу вычислить это выражение. Проверьте правильность написания!');
                return;
            }
    ...
    // Базовые ответы (должны быть определены!)
...
Что из этого тебя интересует больше всего?`;
    ...
    setupEventListeners() {
        ...
        // Загружаем контекст при инициализации
        this.loadContext();
    }
}
